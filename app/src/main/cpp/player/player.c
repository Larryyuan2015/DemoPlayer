...
void video_refresh_timer(void *userdata) {
    VideoState *is = (VideoState *) userdata;
    //VideoPicture *vp;
    LOGI("video_refresh_timer start:xxxxxxxxxxxx" );
    if (is->video_st) {
        if (is->pictq_size == 0) {
...
int decode_thread(void *arg) {
	...
	LOGI("decode_thread file_path:%s", is->filename);
